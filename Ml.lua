...
local Window = Rayfield:CreateWindow({
   ...
   LoadingTitle = "Divine Hub Key System",
   ...
   KeySettings = {
      ...
      Subtitle = "Enter the official key to access",
      Note = "Creator: @Sulfrax",
      FileName = "DivineKeyConfig_Reset",
      SaveKey = false, 
...
-- --- TAB 2: STATS (FIXATO: SUPPORTO INTEGRALE AL RESET A 0 DI ENDURANCE/MOBILITY) ---
...
local function getMuscleStat(statName)
    -- 1. Controlla cartella dati leaderstats principale
    ...
    -- 2. Scansione dinamica configurazioni interne salvate dal server
    ...
    -- 3. Estrazione diretta dei valori grafici dallo schermo (con filtro pulizia testo)
    for _, gui in ipairs(PlayerGui:GetChildren()) do
        if gui:IsA("ScreenGui") and gui.Enabled then
            for _, textLabel in ipairs(gui:GetDescendants()) do
                if textLabel:IsA("TextLabel") and textLabel.Visible then
                    local labelName = string.lower(textLabel.Name)
                    local labelText = string.lower(textLabel.Text)
                    local target = string.lower(statName)
                    
                    if string.find(labelName, target) or string.find(labelText, target) then
                        local numberMatch = textLabel.Text:match("[%d%.%,%s%a%+/]+")
                        if numberMatch and numberMatch ~= "" then 
                            return numberMatch 
                        end
                    end
                end
...
task.spawn(function()
    while true do
        local str = getMuscleStat("Strength") or getMuscleStat("Muscles") or "0"
        local endur = getMuscleStat("Endurance") or getMuscleStat("Durability") or "0"
        local mob = getMuscleStat("Mobility") or getMuscleStat("Agility") or getMuscleStat("Speed") or "0"
        local reb = getMuscleStat("Rebirths") or getMuscleStat("Rebirth") or "0"
        local kils = getMuscleStat("Kills") or getMuscleStat("Kill") or "0"

        -- Sincronizzazione immediata sul pannello Rayfield (gestisce il reset a 0)
